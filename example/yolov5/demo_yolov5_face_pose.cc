...
static const int kOutputCandidates = 25200;
// model config
...
const static vector<string> CLASSES = {"face"};
...
vector<thor::Detection> doPostProcess(float *out,
  ...
  for (int i = 0; i < kOutputCandidates; ++i) {
    ...
    float conf_i = out[i * (15 + CLASSES.size()) + 4];
    int label_i =0;
    float prob_i = out[i * (15 + CLASSES.size()) + 15];
    conf_i *= prob_i;
    if (conf_i >= 0.3) {

//      // model outputs in cx,cy,w,h format
        float cx = out[i * (15 + CLASSES.size()) + 0] - pad_x;
        float cy = out[i * (15 + CLASSES.size()) + 1] - pad_y;
        float w = out[i * (15 + CLASSES.size()) + 2];
        float h = out[i * (15 + CLASSES.size()) + 3];
      ...
      {
          thor::landmarks mask;
          mask.x=(out[i * (15 + CLASSES.size()) + 5 +pose*2] - pad_x)/gain;
          mask.y=(out[i * (15 + CLASSES.size()) + 6 +pose*2] - pad_y)/gain;
          det.marks[pose]=mask;
...
int run(int argc, char **argv) {
 // string engine_f = argv[1];
  string engine_f ="/home/py/code/mana/yolov5_face/runs/train/exp17/weights/yolov5_face.trt";
 // string data_f = argv[2];
  string data_f ="/home/py/Videos/recognize.mp4";
  string mode = "fp16";
  string batch_f;
//  if (argc >= 4) {
//    mode = argv[3];
...
//                    0.38, true);
      ...
      for (size_t j = 0; j < 5; j++) {
        cv::Point2f pt = cv::Point2f(dets[i].marks[j].x ,
                                     dets[i].marks[j].y );
        cv::circle(res, pt, 1, cv::Scalar(0, 255, 0), 2);
      }
...
//      cv::imshow("YoloV5 TensorRT rstg", pr_img);
    ...
    for (auto &f: fn) {
      ...
      cv::Mat res = thor::vis::VisualizeDetections(frame, dets, CLASSES, nullptr, 1,
                                                   0.38, true);
      for (size_t j = 0; j < 5; j++) {
        cv::Point2f pt = cv::Point2f(dets[j].marks->x ,
                                     dets[j].marks->y );
        cv::circle(res, pt, 1, cv::Scalar(0, 255, 0), 2);
      }
      cv::imshow("YoloV5 TensorRT", res);
