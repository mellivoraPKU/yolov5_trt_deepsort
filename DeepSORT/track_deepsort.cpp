...
//        卡尔曼滤波更新
 ...
 cv::Mat track_deepsort::display(cv::Mat frame)
 {
     ...
     {
         cv::Point point(mytracker.tracks[i].latest_detection.tlwh[0],mytracker.tracks[i].latest_detection.tlwh[1]);
         string id =std::to_string(mytracker.tracks[i].track_id);
         cv::putText(frame, id, point, cv::FONT_HERSHEY_COMPLEX, 2.0,
                     cv::Scalar(0, 255, 0), 2);
         for (int j=1;j<mytracker.tracks[i].trail_list.size();j++)
         {


           // cv::circle(frame, mytracker.tracks[i].trail_list[j], 3, cv::Scalar(rng.uniform(0,255),rng.uniform(0,255),rng.uniform(0,255)));
            cv::line(frame, mytracker.tracks[i].trail_list[j-1], mytracker.tracks[i].trail_list[j], cv::Scalar(0,0,255), 8);
